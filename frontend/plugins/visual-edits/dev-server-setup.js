...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Removed Emergent-specific allow-listing


      // Removed Emergent-related allow-listing
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@educompete.org" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@educompete.org" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
